...
namespace Recon_proto.Controllers
{
	public class DataSetController : Controller
	{
			...
			try
			{
                using (var client = new HttpClient())
                {
                    ...
                    for (int i = 0; i < result.Rows.Count; i++)
                    {
                        ...
                        objcat.source_db_type = result.Rows[i]["source_db_type"].ToString();
                        objcat_lst.Add(objcat);
                    }
                    return Json(objcat_lst);
                }
        ...
        [HttpPost]
        public JsonResult Datasetindex([FromBody] Datasetindexmodel context)
        {
            urlstring = _configuration.GetSection("Appsettings")["apiurl"].ToString();
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpContent content = new StringContent(JsonConvert.SerializeObject(context), UTF8Encoding.UTF8, "application/json");
                    var response = client.PostAsync("Datasetindex", content).Result;
                    Stream data = response.Content.ReadAsStreamAsync().Result;
                    StreamReader reader = new StreamReader(data);
                    post_data = reader.ReadToEnd();
                    string d2 = JsonConvert.DeserializeObject<string>(post_data);
                    ...
                    for (int i = 0; i < result.Rows.Count; i++)
                    {
                        objList.in_datasetindex_gid = Convert.ToInt32(result.Rows[i]["in_datasetindex_gid"]);
                        objList.out_msg = result.Rows[i]["out_msg"].ToString();
                        objList.out_result = result.Rows[i]["out_result"].ToString();
                    }
                    return Json(objList);
            ...
            {
                CommonController objcom = new CommonController(_configuration);
                objcom.errorlog(ex.Message, "Datasetindex");
                return Json(ex.Message);
            }
        }
        ...
        #region Datasetindexlist

        [HttpPost]
        public JsonResult Datasetindexlist([FromBody] Datasetindexfieldmodel context)
        {
            urlstring = _configuration.GetSection("Appsettings")["apiurl"].ToString();
            DataTable result = new DataTable();
            string post_data = "";
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpContent content = new StringContent(JsonConvert.SerializeObject(context), UTF8Encoding.UTF8, "application/json");
                    var response = client.PostAsync("Datasetindexlist", content).Result;
                    Stream data = response.Content.ReadAsStreamAsync().Result;
                    StreamReader reader = new StreamReader(data);
                    post_data = reader.ReadToEnd();
                    string d2 = JsonConvert.DeserializeObject<string>(post_data);
        ...
        public class Datasetindexfieldmodel
        {
            public string? in_datasetindex_code { get; set; }
            public string? in_user_code { get; set; }
        }
